...
export function ModerationAppealsQueueScreen({ navigation }) {
  ...
  const load = useCallback(async ({ refresh = false } = {}) => {
    ...
    try {
      const rows = await fetchModerationAppealQueue({ status });
      setAppeals(rows);
      setError('');
    } catch (loadError) {
      setError(loadError?.message || 'Could not load enforcement appeals.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.screen}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle}>Enforcement appeals</Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[styles.content, appeals.length === 0 && styles.emptyContent]}
          ...
          ListEmptyComponent={(
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>No matching appeals</Text>
              <Text style={styles.emptyText}>This appeal queue is currently clear.</Text>
            </View>
          )}
          renderItem={({ item }) => (
            <Pressable
              onPress={() => navigation.navigate('ModerationAppealDetail', { appealId: item.appealId })}
            ...
            >
              ...
              <View style={styles.cardCopy}>
                ...
                <Text style={styles.meta}>
                  {item.enforcementState === 'suspended' ? 'Suspension' : 'Restriction'} · {APPEAL_STATUS_LABELS[item.status] || item.status}
                </Text>
                <Text style={styles.meta}>{formatDate(item.submittedAt)}</Text>
                {!item.enforcementActive && item.status !== 'resolved' ? (
                  <Text style={styles.inactive}>Action is no longer active</Text>
                ) : null}
              </View>
...
const styles = StyleSheet.create({
  ...
  inactive: { marginTop: 4, fontFamily: 'Manrope_700Bold', fontSize: 10, color: '#9b2c2c' },
