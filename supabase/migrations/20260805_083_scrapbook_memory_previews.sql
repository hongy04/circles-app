-- Circles Step 28 — scrapbook previews for event memories
--
-- Extends the existing Circle event-list read model with up to three ready
-- event-gallery photo paths. The app signs those paths in the same batch as
-- custom event covers, so past-event memory cards can show scrapbook peeks
-- without issuing one gallery request per event.
